...
const Modal = (props) => {
  ...
  const addExpenseHandler = (e) => {
    if (
      ...
      e.target.className.includes('incomeBtn') &&
        props.onAddExpense({ ...inputData, transaction: 'income' });
      e.target.className.includes('expenseBtn') &&
        props.onAddExpense({ ...inputData, transaction: 'expense' });
      props.onHideModal();
    }
  };
  ...
  return (
    <>
      ...
      <div className='card mt-3 position-absolute t-40 start-50 translate-middle w-50 m-auto'>
        <div className='card-header display-6 fw-semibold'>
          Add {props.type}
        </div>
